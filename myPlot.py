...
def annotateBoxplot(
    bpdict, ax, pctStrList, pctHideStrList, x_offset=0.05, x_loc=0, annotate_params=None
):
...
def drawBoxplotPy(
    ...
    assert (
        numChartType == 1 or numChartType == 2
    ), "1 or 2 is only supported for nuChartType ..."
    ...
    for (colIdx, col) in enumerate(dfList[0].columns):
        ...
        for chartTypeIdx in list(range(numChartType)):
            ...
            if chartTypeIdx == 0:
                drawSubBoxplot(
                    df, axes[colIdx, chartTypeIdx], pctAddList, pctHideStrList
                )
            if chartTypeIdx == 1:
